...
void Send_Resource(int socket, char * URI, const char *method) {
  ...
  // Get the Content-Type. This looks at the file extensions. Can handle html, javascript, css. Default is plain. 
  ...
  // Header now has Content-Type, and Content-Length. Previously, it was only Content-Length. 
  snprintf(headers, sizeof(headers), "Content-Type: %s\r\nContent-Length: %ld\r\n\r\n", content_type, sz);
  send(socket, headers, strlen(headers), 0);

  // Consider whether the request method is GET, or HEAD. 
