...
int main(int argc, char *argv[])
{
  ...
  if(listen_socket == -1) {
    perror("1. Error creating socket");
    exit(EXIT_FAILURE);
  }
  /* END CODE SNIPPET 1 */
  ...
  if(listen(listen_socket, 20) < 0) { // I arbitrarily chose 20. 
    perror("4. Listening Error");
    exit(EXIT_FAILURE);
  }
  /* END CODE SNIPPET 4 */
  ...
  while (listen_socket >= 0)
  {
    ...
    int c = sizeof(client_address);
    connection_socket = accept(listen_socket, (struct sockaddr *)&client_address, &c);
    
    if(connection_socket < 0) {
      perror("5. Can't accept connection");
      continue;
    }
    /* END CODE SNIPPET 5 */
    ...
    if ((pid = fork()) == 0)
    {
      ...
      // If it's a real method, but hasn't been implemented, I return 501. If it's an invalid, unrecognised method, I return 400.
      } else if(strcmp(new_request.method, "POST") == 0 || strcmp(new_request.method, "DELETE") == 0 || strcmp(new_request.method, "PUT") == 0 || strcmp(new_request.method, "PATCH") == 0) {
        status_code = 501;
      ...
      if(status_code == 200) { // Must have receieved an OK. 
        // This results in Send_Resource being called, regardless of the method being HEAD or GET. 
