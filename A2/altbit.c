...
{
  /* Ive converted checksum to an unsigned int, and cast it back to an int when I return. */
  unsigned int checksum = 0;
  int i;
  /****** 4. FILL IN CODE to calculate the checksum of packet *****/
  /* I add the header values to the checksum first. ACK and Seq num. */
  checksum = checksum + packet.seqnum + packet.acknum;
  
  for(i = 0; i < 20; i++) {
    /* To keep data integrity, I incorporate the packet's payload into the checksum. */
    /* I do a LEFT ROTATION before I sum the packet's bytes. Idea is basically to make the checksum more sensitive. */
    checksum = (checksum << 1) | (checksum >> (sizeof(unsigned int)*8 - 1));
...
void A_output(struct msg message)
{
  ...
  if ( windowcount < WINDOWSIZE) {
    ...
    /* windowlast will always be 0 for alternating bit*/
    ...
    /**** When A sends a packet, it also has to START a timer. *****/
    starttimer(A, RTT);
...
*/
static int prevACK = 1;  /* Storing previous ACK (prevACK) to keep track of state*/
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (packet.acknum != prevACK) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      /* packet is a new ACK */
      if (TRACE > 0)
        printf("----A: ACK %d is not a duplicate\n",packet.acknum);
      new_ACKs++;
      prevACK = packet.acknum; /* Update the value of previous ACK*/
...
void A_timerinterrupt(void)
{
  ...
  starttimer(A, RTT); /**** Must start the timer again, because packet's been retransmitted. *****/
  packets_resent++;
}       
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))  && (packet.seqnum == expectedseqnum) ) {
    ...
    /***** 3. FILL IN CODE  What ACK number should be sent if the packet
      ...
      /* Sends the ack number, of the last correctly received packet*/ 
